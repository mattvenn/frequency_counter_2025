...
test_edge_detect:
	...
	MODULE=test.test_edge_detect vvp -M $$(cocotb-config --prefix)/cocotb/libs -m libcocotbvpi_icarus sim_build/sim.vvp
...
test_frequency_counter:
	...
	TESTCASE=test_all MODULE=test.test_frequency_counter vvp -M $$(cocotb-config --prefix)/cocotb/libs -m libcocotbvpi_icarus sim_build/sim.vvp
	! grep failure results.xml

test_frequency_counter_with_period:
	rm -rf sim_build/; mkdir sim_build/
	iverilog -o sim_build/sim.vvp -s frequency_counter -s dump -g2012 src/frequency_counter.v src/edge_detect.v src/seven_segment.v test/dump_frequency_counter.v 
	MODULE=test.test_frequency_counter vvp -M $$(cocotb-config --prefix)/cocotb/libs -m libcocotbvpi_icarus sim_build/sim.vvp
	! grep failure results.xml

test_seven_segment:
	rm -rf sim_build/; mkdir sim_build/
	iverilog -o sim_build/sim.vvp -s seven_segment -s dump -g2012 src/seven_segment.v test/dump_seven_segment.v 
	MODULE=test.test_seven_segment vvp -M $$(cocotb-config --prefix)/cocotb/libs -m libcocotbvpi_icarus sim_build/sim.vvp
	! grep failure results.xml
